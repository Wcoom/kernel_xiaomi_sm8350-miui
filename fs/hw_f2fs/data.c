...
#if (LINUX_VERSION_CODE > KERNEL_VERSION(5, 10, 0)) && defined(CONFIG_F2FS_FS_DISK_TURBO)
				     ...
				     unsigned int offs)
{
	...
	unsigned int i;
	int err;
	struct inode *inner = get_inner_inode(inode);
	...
	for (i = offs; i < offs + len; i += blocksize, lblk_num++) {
		err = fscrypt_encrypt_block_inplace(inner ? : inode, page,
...
#endif
#else
	bio = f2fs_bio_alloc(sbi, min_t(int, nr_pages, BIO_MAX_PAGES), for_write);
#endif
	if (!bio)
...
#ifdef CONFIG_F2FS_FS_DISK_TURBO
				f2fs_rw_hint_to_seg_type(inode->i_write_hint),true);
#else
