...
/* used for distinguish func name between bootimage and camera.ko */
#ifdef HIVIEW_FUNC_NO_PREFIX
#define MUTABLE_FUNC_NAME(func) func
#else
#define MUTABLE_FUNC_NAME(func) camkit_##func
#endif
...
static const struct driver_ic_position_info
	g_driver_ic_position_info[CAMERA_POSITION_MAX] = {
...
static const struct driver_error_type_info
	g_driver_error_type_info[DRIVER_ERROR_TYPE_MAX] = {
...
int MUTABLE_FUNC_NAME(hiview_init)(struct camera_dmd_info* info)
{
...
EXPORT_SYMBOL(MUTABLE_FUNC_NAME(hiview_init));
...
void MUTABLE_FUNC_NAME(hiview_report)(struct camera_dmd_info* cam_info)
{
	...
	for (index = 0; index < CAMERA_POSITION_MAX; index++)
		if (g_driver_ic_position_info[index].ic_position == cam_info->ic_position) {
			...
			loge_if(strncat_s(error_info, HIVIEW_MAX_CONTENT_LEN,
				g_driver_ic_position_info[index].ic_position_info,
				HIVIEW_MAX_CONTENT_LEN - strlen(error_info) - 1));
			break;
		}
	...
	if (strlen(cam_info->extra_info.module_name)) {
		...
		loge_if(strncat_s(error_info, HIVIEW_MAX_CONTENT_LEN, "from ",
			HIVIEW_MAX_CONTENT_LEN - strlen(error_info) - 1));
		loge_if(strncat_s(error_info, HIVIEW_MAX_CONTENT_LEN,
			cam_info->extra_info.module_name,
			HIVIEW_MAX_CONTENT_LEN - strlen(error_info) - 1));
		loge_if(strncat_s(error_info, HIVIEW_MAX_CONTENT_LEN, " ",
			HIVIEW_MAX_CONTENT_LEN - strlen(error_info) - 1));
	}
	...
	if (type < DRIVER_ERROR_TYPE_MAX)
		loge_if(strncat_s(error_info, HIVIEW_MAX_CONTENT_LEN,
			g_driver_error_type_info[type].error_type_info,
			HIVIEW_MAX_CONTENT_LEN - strlen(error_info) - 1));

	if (strlen(cam_info->extra_info.error_scene)) {
		loge_if(strncat_s(error_info, HIVIEW_MAX_CONTENT_LEN,
			cam_info->extra_info.error_scene,
			HIVIEW_MAX_CONTENT_LEN - strlen(error_info) - 1));
	}
...
EXPORT_SYMBOL(MUTABLE_FUNC_NAME(hiview_report));
