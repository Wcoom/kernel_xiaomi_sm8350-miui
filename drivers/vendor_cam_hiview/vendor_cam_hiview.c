	...
	struct cam_sensor_ctrl_t* s_ctrl, const char* error_scene)
{
	...
	if (s_ctrl) {
		cam_info.ic_position = s_ctrl->sensordata->slave_info.sensor_probe_info.position_id;
	...
	const char* error_scene)
{
	...
	if (io_master_info->not_need_dmd_report) {
		log_inf("not need to report i2c error dmd");
		return;
	}
	...
	case CAM_EEPROM:
		...
		s_ctrl = vendor_get_sensor_ctrl(e_ctrl->soc_info.index, CAM_EEPROM);
		break;
	case CAM_OIS:
		error_type = OIS_I2C_ERR;
		o_ctrl = container_of(io_master_info, struct cam_ois_ctrl_t, io_master_info);
		s_ctrl = vendor_get_sensor_ctrl(o_ctrl->soc_info.index, CAM_OIS);
		break;
	default:
	...
	return vendor_cam_hiview_handle(error_type, s_ctrl, error_scene);
}
