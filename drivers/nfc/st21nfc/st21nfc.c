				 ...
				 size_t count, loff_t *offset)
{
	...
	if (is_active_cmd) {
		mutex_lock(&st21nfc_dev->platform_data.read_mutex);
		if (g_activeAidVal.type == 0 && count > INDEX_ACTIVE_FLAG
			&& data_buf[INDEX_ACTIVE_FLAG] == 0x4F) {
			pr_info("%s: active flag no set, change 4F->FF\n", __func__);
			data_buf[INDEX_ACTIVE_FLAG] = 0xFF;
		}
		mutex_unlock(&st21nfc_dev->platform_data.read_mutex);
	}
	/* Send data */
	for (retry = 0; retry < NFC_TRY_NUM; retry++) {
		...
		hwlog_info("%s : retry = %d, ret = %d, expect count = %3d, header: %02x%02x%02x%02x%02x\n",
			__func__, retry, ret, (int)count, data_buf[0],
			data_buf[1], data_buf[2], data_buf[3], data_buf[4]);
		if (ret == (int)count) {
			isSuccess = true;
			break;
		} else {
