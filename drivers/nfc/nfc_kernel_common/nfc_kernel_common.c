...
{
	return (ssize_t)(snprintf_s(buf, strlen(nfc_brcm_config_name) + 1, strlen(nfc_brcm_config_name), "%s", nfc_brcm_config_name));
}
...
{
	return (ssize_t)(snprintf_s(buf, MAX_ATTRIBUTE_BUFFER_SIZE, MAX_ATTRIBUTE_BUFFER_SIZE - 1, "%d", g_nfc_single_channel));
}
...
{
	return (ssize_t)(snprintf_s(buf, sizeof(g_nfc_switch_state) + 1, sizeof(g_nfc_switch_state), "%d", g_nfc_switch_state));
}
...
{
	return (ssize_t)(snprintf_s(buf, MAX_ATTRIBUTE_BUFFER_SIZE, MAX_ATTRIBUTE_BUFFER_SIZE - 1, "%d", g_nfc_activated_se_info));
}
