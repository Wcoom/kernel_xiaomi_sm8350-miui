...
static struct lbs_power_vreg_data *lbs_power_pdata =NULL;
...
static int bcm_gps_vreg_init(struct device *dev,struct lbs_power_vreg_data *vreg)
 {
 	int rc = 0;
 
 	pr_err(" vreg_get for : %s", vreg->name);
 ...
 static int bcm_gps_vreg_enable(struct lbs_power_vreg_data *vreg)
 {
 	int rc = 0;
 	...
 	if (!vreg->is_enabled) {
 		if (vreg->set_voltage_sup) {
 			...
 			if (rc < 0) {
 				pr_err(" vreg_set_vol(%s) failed rc=%d", vreg->name, rc);
 				goto out;
 			}
 		}
 		...
 		if (vreg->load_uA >= 0) {
 			...
 			if (rc < 0) {
 				pr_err(" vreg_set_mode(%s) failed rc=%d", vreg->name, rc);
 				goto out;
 			}
 		}
 	...
 	pr_err(" vreg_en successful for : %s", vreg->name);
 out:
 	return rc;
 }
 
 static int bcm_gps_configure_vreg(struct device *dev,struct lbs_power_vreg_data *vreg)
 {
 	int rc = 0;
 
 	pr_err(" config %s", vreg->name);
 
 	/* Get the regulator handle for vreg */
 	if (!(vreg->reg)) {
 		rc = bcm_gps_vreg_init(dev,vreg);
 		if (rc < 0)
 			return rc;
 	}
 	rc = bcm_gps_vreg_enable(vreg);
 
 	return rc;
 }

static int bcm_gps_vreg_disable(struct lbs_power_vreg_data *vreg)
{
 int rc = 0;
 ...
 if (vreg->is_enabled) {
	 ...
	 if (rc < 0) {
		 pr_err(" regulator_disable(%s) failed. rc=%d", vreg->name, rc);
		 goto out;
	 }
 ...
 pr_err(" vreg_disable successful for : %s", vreg->name);
out:
 return rc;
}

#define MAX_PROP_SIZE 32
static int bcm_gps_dt_parse_vreg_info(struct device *dev,
	  struct lbs_power_vreg_data **vreg_data, const char *vreg_name)
{
  ...
  pr_err(" vreg dev tree parse for %s", vreg_name);

  *vreg_data = NULL;
  ret = snprintf(prop_name, MAX_PROP_SIZE, "%s-supply", vreg_name);
  if (ret < 0) {
	  pr_err(" %s property is not valid", vreg_name);
  }
  if (of_parse_phandle(np, prop_name, 0)) {
	  vreg = devm_kzalloc(dev, sizeof(*vreg), GFP_KERNEL);
	  if (!vreg) {
		 pr_err(" No memory for vreg: %s", vreg_name);
		  ret = -ENOMEM;
		  goto err;
	  }
	  ...
	  if (ret < 0) {
		  pr_err(" %s property is not valid", prop_name);
		  vreg->load_uA = -1;
		  ret = 0;
	  }

	  *vreg_data = vreg;
	  pr_err(" %s: vol=[%d %d]uV, current=[%d]uA",
		  vreg->name, vreg->low_vol_level,
		  vreg->high_vol_level,
		  vreg->load_uA);
  } else
	  pr_err(" %s: is not provided in device tree", vreg_name);

err:
  return ret;
}

static int bcm_gps_power(struct device *dev,int on)
{
	int rc = 0;
	pr_err(" on: %d", on);

	if (lbs_power_pdata == NULL) {
		rc = bcm_gps_dt_parse_vreg_info(dev, &lbs_power_pdata, "lbs,lbs-vdd-spmu");
		if (rc) {
			pr_err(" parse err %d\n", rc);
			return rc;
		}
	 ...
	 if (on == 1) {
		 // Power On, just pm7325_l11 1.8V
		 if (lbs_power_pdata) {
			 rc = bcm_gps_configure_vreg(dev,lbs_power_pdata);
			 if (rc < 0) {
				 pr_err(" bs_power lbs_vdd_spmu config failed");
				 goto out;
			 }
		 }
	 } else if (on == 0) {
		 if (lbs_power_pdata) {
			 rc = bcm_gps_vreg_disable(lbs_power_pdata);
			 if (rc < 0) {
				 pr_err("lbs_power lbs_vdd_spmu config failed");
				 goto out;
			 }
		 }
	 } else {
		 pr_err("Invalid power mode: %d", on);
		 rc = -1;
	 }
  out:
	 return rc;
...
static int bcm_spi_probe(struct spi_device *spi)
{
	...
	skip_validity_check = of_property_read_bool(spi->dev.of_node,"ssp-skip-validity-check");
	...
	if (ret) {
		printk(KERN_EMERG "failed to request HOST REQ, ret:%d", ret);
		goto err_exit;
	}
	spi->irq = gpio_to_irq(host_req);
	if (spi->irq < 0) {
		printk(KERN_EMERG "irq=%d for host_req=%d not correct\n",
				spi->irq, host_req);
		goto err_exit;
	}
	/* Config GPIO */
	...
	if (ret) {
		printk(KERN_EMERG "failed to request MCU REQ, ret:%d", ret);
		goto err_exit;
	}
	ret = gpio_direction_output(mcu_req, 0);
	if (ret) {
		printk(KERN_EMERG "failed set MCU REQ as input mode, ret:%d",
				ret);
		goto err_exit;
	}
	ret = gpio_request(mcu_resp, "MCU RESP");
	if (ret) {
		printk(KERN_EMERG "failed to request MCU RESP, ret:%d", ret);
		goto err_exit;
	}
	ret = gpio_direction_input(mcu_resp);
	if (ret) {
		printk(KERN_EMERG "failed set MCU RESP as input mode, ret:%d",
				ret);
		goto err_exit;
	}
	...
	if (ret) {
		printk(KERN_EMERG "failed to request GPS NSTANDBY, ret:%d", ret);
		goto err_exit;
	}
	ret = gpio_direction_output(nstandby, 0);
	if (ret) {
		printk(KERN_EMERG "failed set GPS NSTANDBY as out mode, ret:%d",
				ret);
		goto err_exit;
	}
	/*if (power_gpio_config_output(spi->dev.of_node, "mcu-req-gpios", "MCU REQ",mcu_req, 0))
	{
		printk(KERN_EMERG "failed set MCU REQ\n");
		goto err_exit;
	}
	if (power_gpio_config_input(spi->dev.of_node, "mcu-resp-gpios", "MCU RESP",mcu_resp, 0))
	{
		printk(KERN_EMERG "failed set MCU RESP\n");
		goto err_exit;
	}
	if (power_gpio_config_output(spi->dev.of_node, "nstandby-gpios", "GPS NSTANDBY",nstandby, 0))
	{
		printk(KERN_EMERG "failed set GPS NSTANDBY\n");
		goto err_exit;
	}*/
	/* enable gps_power */
