...
void mm_ion_detail_test(size_t len)
{
	int k;
	size_t size;
	struct mm_detail_info *buf = NULL;
...
void mm_ashmem_detail_test(size_t len)
{
	int k;
	size_t size;
	struct mm_detail_info *buf = NULL;

	pr_err("========get ashmem info start==========\n");
	buf = vmalloc(len * sizeof(*buf));
	if (!buf)
		return;

	size = get_ashmem_detail((void *)buf, len);
	for (k = 0; k < min(len, size); k++)
		pr_err("pid:%d, %zu kB\n\n",
			(buf + k)->pid, (buf + k)->size / SZ_1K);

	vfree(buf);
	pr_err("========get ashmem info end==========\n");

	pr_err("========get ashmem stats start==========\n");
	pr_err("total size:%zu kB\n", get_stats_ashmem() / SZ_1K);
	pr_err("========get ashmem stats end==========\n");
}
