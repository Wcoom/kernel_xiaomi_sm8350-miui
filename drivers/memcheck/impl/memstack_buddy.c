...
static int buddy_track_on_flag;
static int buddy_track_open_flag;
	...
	unsigned int order, unsigned long caller)
{
	...
	if (!buddy_track_on_flag || !buddy_global_init || !page)
		return;
...
int __init mm_buddy_track_map(int nid)
{
	phys_addr_t size;
	unsigned long flags;
	void *base = NULL;

#if defined(CONFIG_QCOM_MEM_OFFLINE)
...
int mm_buddy_track_unmap(void)
{
	...
	spin_lock_irqsave(&buddy_lock, flags);
	if (!buddy_track_on_flag) {
		spin_unlock_irqrestore(&buddy_lock, flags);
		return 0;
	}

	buddy_track_on_flag = 0;
	spin_unlock_irqrestore(&buddy_lock, flags);
...
int buddy_stack_open(int type)
{
	...
	if (buddy_track_open_flag) {
		mutex_unlock(&buddy_mutex);
		return 0;
	}
	buddy_fetch_stack(type);
	buddy_track_open_flag = 1;
	mutex_unlock(&buddy_mutex);
...
{
	mutex_lock(&buddy_mutex);
	buddy_track_open_flag = 0;
	mutex_unlock(&buddy_mutex);
	return 0;
}

size_t buddy_stack_read(
	struct mm_stack_info *buf, size_t len, int type)
{
	size_t cnt;
